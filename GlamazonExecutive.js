...
var createDepartment = function(){
	...
	var newDepartment = {
		properties: {
			newDeptName:{ description: colors.magenta('Please enter the name of the new department you would like to add (better be sexy, though.')
			},
			newOverhead:{ description: colors.magenta('What are the overhead costs for this fine a$$ department?')
			},
		}
	}
	...
	prompt.get(newDepartment, function(err, res){
		...
		connection.query('INSERT INTO Departments (DepartmentName, OverHeadCosts, TotalSales) VALUES (?, ?, ?);', [newDept[0].deptName, newDept[0].overHeadNew, newDept[0].autoTotalSales], function(err, result){
			if(err){
				...
				console.log('');
				console.log(colors.blue.underline('New Department sucessfully created. Make it sexy!'));
				console.log(' ');
				connection.end();
			}
		})
