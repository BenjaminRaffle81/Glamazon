...
var purchase = function(){
	...
	var productInfo = {
		properties: {
			itemID:{description: colors.blue('Please enter the ID # of the gorj item you wish to purchase!')},
			Quantity:{description: colors.green('How many fine-a$$ items would you like to purchase?')}
		},
	};
	...
	prompt.get(productInfo, function(err, res){
		...
		connection.query('SELECT * FROM Products WHERE ItemID=?', productPurchased[0].itemID, function(err, res){
				...
				if(res[0].StockQuantity < productPurchased[0].Quantity){
					console.log('That product is out of stock! Sorry, you sexy beast...');
					connection.end();
					...
					connection.query("UPDATE Products SET StockQuantity = " + newQuantity +" WHERE ItemID = " + productPurchased[0].itemID, function(err, res){
						...
						console.log('');
						console.log(colors.cyan('Your order has been processed.  Stay fab, boo!'));
						console.log('');
